...
USBH_StatusTypeDef USBH_Get_DevDesc (USBH_HandleTypeDef *phost, uint8_t length)
{
        ...
        if ((status = USBH_GetDescriptor (phost, USB_REQ_RECIPIENT_DEVICE | USB_REQ_TYPE_STANDARD, USB_DESC_DEVICE, phost->device.Data, length))
            == USBH_OK) {
...
#endif
        if ((status = USBH_GetDescriptor (phost, USB_REQ_RECIPIENT_DEVICE | USB_REQ_TYPE_STANDARD, USB_DESC_CONFIGURATION, pData, length))
            == USBH_OK) {
...
{
        USBH_StatusTypeDef status;
        if ((status = USBH_GetDescriptor (phost, USB_REQ_RECIPIENT_DEVICE | USB_REQ_TYPE_STANDARD, USB_DESC_STRING | string_index,
...
static void USBH_ParseDevDesc (USBH_DevDescTypeDef *dev_desc, uint8_t *buf, uint16_t length)
{
        ...
        if (length > 8) { /* For 1st time after device connection, Host may issue only 8 bytes for
...
static void USBH_ParseStringDesc (uint8_t *psrc, uint8_t *pdest, uint16_t length)
{
        ...
        */
        ...
        if (psrc[1] == USB_DESC_TYPE_STRING) { /* Make sure the Descriptor is String Type */

                /* psrc[0] contains Size of Descriptor, subtract 2 to get the length of string */
                strlength = (((psrc[0] - 2) <= length) ? (psrc[0] - 2) : length);
                psrc += 2; /* Adjust the offset ignoring the String Len and Descriptor type */

                for (idx = 0; idx < strlength; idx += 2) { /* Copy Only the string and ignore the UNICODE ID, hence add the src */
                        *pdest = psrc[idx];
                        pdest++;
                }
...
static USBH_StatusTypeDef USBH_HandleControl (USBH_HandleTypeDef *phost)
{
        ...
        case CTRL_DATA_OUT:
                ...
                USBH_CtlSendData (phost, phost->Control.buff, phost->Control.length, phost->Control.pipe_out, 1);
                phost->Control.timer = phost->Timer;
